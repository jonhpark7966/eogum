...
| 파일 | 현재 문제 | 우선 분리 대상 |
|------|-----------|----------------|
| [projects.py](/home/jonhpark/workspace/eogum/apps/api/src/eogum/routes/projects.py) | 프로젝트 CRUD, credit check, queue enqueue, 후처리 worker 시작이 한 파일에 섞여 있음 | `projects` 서비스, `processing.reprocess` 서비스 |
...
| [evaluations.py](/home/jonhpark/workspace/eogum/apps/api/src/eogum/routes/evaluations.py) | engine-native review payload 소비, 평가 저장, 버전 조회, 메트릭 계산이 한 파일에 있음 | `evaluations.segments`, `evaluations.service`, `evaluations.metrics` |
| [avid.py](/home/jonhpark/workspace/eogum/apps/api/src/eogum/services/avid.py) | CLI 실행, JSON payload 해석, 초기 처리/후처리 명령 조립이 한 파일에 섞여 있음 | `adapters.avid_cli`, `processing.report_parser`, `processing.reprocess` |
...
| `processing` | queue, worker, 상태 전이, artifact 등록, 후처리 orchestration | `jobs`, `edit_reports`, `projects.status` | [job_runner.py](/home/jonhpark/workspace/eogum/apps/api/src/eogum/services/job_runner.py) |
...
| `avid_cli` | `avid-cli` subprocess 호출, manifest 해석, 초기 처리와 후처리 명령 진입점 제공 | [avid.py](/home/jonhpark/workspace/eogum/apps/api/src/eogum/services/avid.py) |
...
1. `adapters.avid_cli`
2. `processing.reprocess`
3. `evaluations.metrics`
4. `evaluations.segments`
5. `processing.report_parser`
6. `processing.worker`

이 순서는 `avid` 경계를 먼저 고정하고, 그 다음 초기 처리와 후처리 orchestration 을 분리하기 위한 것이다.
...
3. 해당 단계의 수동 workflow 검증 문서가 갱신돼 있어야 한다.
4. live CLI 또는 API 검증 순서가 문서로 남아 있어야 한다.
